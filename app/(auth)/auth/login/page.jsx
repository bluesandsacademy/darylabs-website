...
export default function LoginPage() {
  ...
  const { setUser, setToken, isLoggedIn } = useUser();

  useEffect(() => {
    document.title = "Welcome Back! | Darylabs — Transforming STEM Education";
  }, []);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (role === "schooladmin") router.push("/school/dashboard");
      else if (role === "globaladmin") router.push("/admin/dashboard");
      else if (role === "teacher") router.push("/teacher/dashboard");
      else router.push("/dashboard");
    } catch (err) {
      console.error("Login failed", err);
      if (err.response?.status === 401) toast.error("Invalid email or password.");
      else toast.error("Login failed. Please try again later.");
    } finally {
      NProgress.done();
    ...
    <section className="min-h-screen p-3 mb-10">
      ...
      >
        <div className="flex flex-col w-full gap-y-1 md:gap-y-4">
          <label htmlFor="emailAddress" className="font-medium text-gray-700 text-sm md:text-md">Email Address</label>
          <input
            type="email"
        ...
        <div className="w-full flex flex-col gap-y-3">
          <Link href="/auth/forgot-password" className="text-gray-500 text-sm md:text-md font-normal text-center">
            Forgot Password?
          </Link>
          <button
            type="submit"
            disabled={isSubmitting}
            className={`text-center rounded-md py-1 md:py-5 bg-[#006fcc] text-white w-full md:text-lg ${isSubmitting ? "opacity-50 cursor-not-allowed" : "hover:bg-blue-700"}`}
          >
            {isSubmitting ? "Logging in..." : "Login"}
          </button>
          <p className="text-gray-500 text-center text-xs md:text-base">
            Don&apos;t have an account?{" "}
            <Link href="/auth/register" className="text-blue-500 underline font-normal">Create Account</Link>
          </p>
        </div>
      </form>
    </section>
